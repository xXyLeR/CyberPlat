...
não na raiz do repositório. Sem configurar isso, a plataforma não acha
o app pra buildar (ou builda a coisa errada) e você vê 404 nas páginas.

**Vercel**: no dashboard do projeto, em Settings → General → **Root
Directory**, defina `web`. Depois, em Settings → Environment Variables,
adicione `NEXT_PUBLIC_API_URL` apontando para a URL real do seu core-api
em produção (nunca `localhost`, que só existe na sua máquina).

**Netlify**: já incluí um `netlify.toml` na raiz do repo com
`base = "web"` e o plugin oficial `@netlify/plugin-nextjs` — não precisa
configurar nada manualmente no dashboard além de
`NEXT_PUBLIC_API_URL` em Site settings → Environment variables.

**Importante sobre `output: "standalone"`**: `web/next.config.js` só
ativa esse modo quando a variável `DOCKER_BUILD=1` está setada (isso
acontece automaticamente dentro do `web/Dockerfile`). Standalone é para
rodar com `node server.js` num container — em Vercel/Netlify ele
**quebra o roteamento de páginas dinâmicas e causa 404**, porque essas
plataformas usam seus próprios adaptadores que esperam o build padrão
...
- **CTF com ranking**: desafios por categoria (web, network, linux,
  ...
  leaderboard individual + por equipe, escopados à organização.
- **93 testes automatizados** cobrindo todas as fases (79 no core-api + 16 no session-gateway).
