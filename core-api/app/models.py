...
# ---------------------------------------------------------------------------
# CTF (item 9 do escopo original)
# ---------------------------------------------------------------------------
#
# Decisão de modelagem: Challenge é DELIBERADAMENTE mais simples que Lab
# — não tem `machines`, não provisiona container. Muitos desafios de CTF
# clássicos (crypto, forense, reverse engineering) são só "aqui está um
# arquivo/enigma, ache a flag", sem precisar de um ambiente isolado.
# Desafios que PRECISAM de um ambiente isolado continuam sendo
# modelados como Lab (ex: um desafio "Web" mais elaborado usa Lab, não
# Challenge). CTFSubmission é uma tabela própria, separada de
# Submission (que é sempre atrelada a um LabInstance) — flags de CTF
# não têm sessão de lab associada.
...
class Challenge(Base):
    ...
    title = Column(String, nullable=False)
    slug = Column(String, nullable=False, unique=True)
    category = Column(String, nullable=False)  # web|network|linux|windows|active_directory|cloud|containers|cryptography|forensics|reverse_engineering
    difficulty = Column(String, default="beginner")
    description = Column(Text, default="")
    points = Column(Integer, default=100)
    flag_hash = Column(String, nullable=False)
    hints = Column(JSON, default=list)  # lista de strings
    status = Column(String, default="draft")  # draft | published
    created_by = Column(String, ForeignKey("users.id"), nullable=True)
    created_at = Column(DateTime, default=utcnow)
