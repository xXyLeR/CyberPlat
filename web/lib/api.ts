...
export type ChallengeSubmitResultOut = {
  correct: boolean;
  points_awarded: number;
};
...
export type TeamLeaderboardEntryOut = {
  rank: number;
  team_name: string;
  total_xp: number;
  member_count: number;
...
export const api = {
  ...
  async submitChallengeFlag(id: string, flagValue: string) {
    return request<ChallengeSubmitResultOut>(`/challenges/${id}/submit`, {
      method: "POST",
      body: JSON.stringify({ flag_value: flagValue }),
    });
  },
  async individualLeaderboard() {
    return request<LeaderboardEntryOut[]>("/leaderboard");
  },
